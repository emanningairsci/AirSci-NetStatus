...
def home(request):
    ...
    #|       being recently added to the DB.
    read_data = readDB()
...
def edit(request):
    #| create a page for editing the hostname, IP, and adding or removing services
    #| TODO: add more logic to handle service paths, usernames, passwords, and hostnames
    read_data = readDB()
    hostname = request.matchdict['hostname']
    host_data = read_data.get_host_status(hostname)
